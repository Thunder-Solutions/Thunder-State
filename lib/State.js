  ...
  constructor({
    ...
    const timeTravel = num => {
      ...
      actions.some((action, idx) => {
        ...
        action.mutations.forEach(mutation => {
          ...
          const allButLastKey = path.slice(0, path.length - 1)
          const lastKey = path[path.length - 1]
          const ref = getValueFromPath(stateProxy, allButLastKey)
    ...
    const evaluateGetters = getters => {
      return Object.keys(getters).reduce((accumulator, key) => {
        ...
        accumulator[key] = evaluatedResult && evaluatedResult.constructor === Object
