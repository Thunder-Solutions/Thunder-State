...
export default class State {
  constructor({
    ...
    // set up computed getters
    const immutableState = createImmutableProxy(state)
    const computedGetters = {}
    const computeValues = () => Object.keys(computed).forEach(key => {
      computedGetters[key] = computed[key](immutableState)
    })
    computeValues() // initialize computed values

    // set up all getters
    this.getters = createImmutableProxy({
      ...state,
      ...computedGetters,
    })
