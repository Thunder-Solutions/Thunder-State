...
export default (name, computed, publicInstance, { userDefinedWatchers, enableDevTools }) => {
  ...
  const prevComputed = Object.keys(computed).reduce((acc, cKey) => {
    const cValue = publicInstance.getters[cKey]
  ...
  return (target, path, _newValue) => {
    ...
    const parent = getValueFromPath(publicInstance.getters, path)
    ...
    const addWatcher = getValueFromPath(publicInstance.watchers, path)
    ...
    const cGetters = publicInstance.getters
    Object.keys(computed).forEach(cKey => {
      ...
      const cAddWatcher = publicInstance.watchers[cKey]
